...
- `↑ ↓`（或 `⌃N ⌃P`）在结果之间选择。
- `← →` 翻到上一章 / 下一章，输入框同步改写。翻到下一章落在第 1 节、显示在顶部；
  翻到上一章落在**最后一节**、显示在底部，方便接着上文往回读。需要在输入框里移动光标时用 `⌘←/→` 或 `⌥←/→`。
- 在右侧预览里用鼠标选经节：点一节、拖过几节，或按住 shift 再点，
  高亮范围和输入框（如 `约翰福音 1:1-2`）一起更新。
- `⏎` 在最前面的阅读窗口里打开（没有则新建），`⇧⏎` 或 `⌘D` 在新阅读窗口里打开，`⌘⏎` 复制所选经文。
- `esc` 先清空输入，再按一次才隐藏；`⌥Space` / `⌘W` 直接隐藏并保留输入，复制之后也保留。
- 底部提示栏的每一项都可以用鼠标点。中文输入法正在选字时，以上按键都交还给输入法。

阅读窗口：`←` `→` 翻章（翻到上一章时显示章末），`⌘D`（或右上角 ⧉ 按钮）把当前经文复制到一个新窗口，
